...
class IncrementingCIFAR(NonstationaryLoader):
    def __init__(self, change_period, batch_size,
        ...
        self.dataset_0 = datasets.CIFAR10('data/cifar10', train=train, download=False,
                                          transform=transform)
        self.dataset = datasets.CIFAR100('data/cifar100', train=train, download=False,
                                         transform=transform)
        ...
        if "smallheads" in kwargs.keys():
            self.smallheads = kwargs['smallheads']
        else:
            self.smallheads = True
    def get_data(self):
        self.current_time += 1
        ...
        mb_labels = torch.Tensor(mb_labels)
        if self.smallheads:
            mb_labels -= self.n_class*task_offset
...
class IncrementingCIFAR100(NonstationaryLoader):
    def __init__(self, change_period, batch_size,
                 n_class=10, use_cifar10=True,
                 train=True, seed=1234, kwargs={}):
        super(IncrementingCIFAR, self).__init__()
